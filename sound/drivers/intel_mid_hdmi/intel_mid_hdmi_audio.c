				...
				int flag_silence)
{
	...
	/*Channel allocation mapping (from CEA861)*/
	switch (frame2.fr2_regx.chnl_cnt + 1) {
	case 2:
		frame3.fr3_regx.chnl_alloc = AL_TWO_ZERO;
		break;
	case 4:
		frame3.fr3_regx.chnl_alloc = AL_THREE_ONE;
		break;
	case 6:
		frame3.fr3_regx.chnl_alloc = AL_FIVE_ONE;
		break;
	case 8:
		frame3.fr3_regx.chnl_alloc = AL_SEVEN_ONE;
		break;
	default:
		frame3.fr3_regx.chnl_alloc = AL_TWO_ZERO;
	}
...
int snd_intelhad_start_silence(struct snd_intelhad *intelhaddata)
{
	...
	for (i = HAD_BUF_TYPE_C; i < HAD_MAX_PERIODS; i++) {
		...
		intelhaddata->buf_info[i].buf_size = (MAX_SZ_ZERO_BUF/2);
					...
					int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		intelhaddata->flag_start = 1;
		reg_ops.hdmi_audio_read_modify(AUD_CONFIG, 1, BIT(0));
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		/*
		 * mdfld_dpms is called in non-blocking context
		 * from hpd_irq_event. __cancel_delayed_work takes
		 * care of this by calling delete_timer in
		 * a-synchronous context.
		 */
		__cancel_delayed_work(&intelhaddata->dummy_audio);
		/* Stop reporting BUFFER_DONE/UNDERRUN to above layers*/
		intelhaddata->flag_start = 0;

		/* Reset HW within Audio unit */
		reg_ops.hdmi_audio_read_modify(AUD_CONFIG, 0, BIT(0));
...
static int snd_intelhad_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	snd_intelhad_init_audio_ctrl(substream, intelhaddata, 0);
