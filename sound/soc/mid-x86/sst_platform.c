...
static int sst_send_compress_device_type(const char *codec_dai_name)
{
	struct intel_sst_card_ops *sstdrv_ops;
	struct snd_sst_params str_params = {0};
	int ret_val;
	pr_err("Compressed type pcm detected");
	sstdrv_ops = kzalloc(sizeof(struct intel_sst_card_ops), GFP_KERNEL);
	if (!sstdrv_ops) {
		pr_err("sst: mem allocation for ops fail\n");
		return -ENOMEM;
	}
	sstdrv_ops->vendor_id = MSIC_VENDOR_ID;
	...
	str_params.ops = STREAM_OPS_COMPRESSED_PATH;
	if (strstr(codec_dai_name, V_HS))
		str_params.device_type = SND_SST_DEVICE_HEADSET;
	else if (strstr(codec_dai_name, V_IHF))
		str_params.device_type = SND_SST_DEVICE_IHF;
	ret_val = sstdrv_ops->pcm_control->open(&str_params);
	pr_debug("Open ret_val = %x\n", ret_val);
	kfree(sstdrv_ops);
	return ret_val;
}
...
static int sst_platform_open(struct snd_pcm_substream *substream)
{
	...
	if (strstr(dai_link->cpu_dai_name, VIRTUAL)) {
		pr_debug("pcm_open for Virt, returning.\n");
		sst_send_compress_device_type(dai_link->codec_dai_name);
		return snd_pcm_hw_constraint_integer(runtime,
			 SNDRV_PCM_HW_PARAM_PERIODS);
	}
...
static int sst_platform_close(struct snd_pcm_substream *substream)
{
	...
	sst_cpu_ctx->active_nonvoice_cnt--;
	if (strstr(dai_link->cpu_dai_name, VIRTUAL)) {
		pr_debug("pcm_close for Virt, returning.\n");
		goto func_exit;
	}
	stream = substream->runtime->private_data;
...
static int sst_platform_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (strstr(dai_link->cpu_dai_name, VIRTUAL)) {
		pr_debug("pcm_preare for Virt, returning.\n");
		return ret_val;
	}
	stream = substream->runtime->private_data;
	str_id = stream->stream_info.str_id;
...
#else
		ret = snd_soc_codec_set_params(codec, SNDRV_PCM_FORMAT_S16_LE);
#endif
		if (ret < 0) {
