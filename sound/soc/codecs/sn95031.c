...
static const  char * const sn95031_jack_debounce_text[] = {"61us", "122us",
					"244us", "488us", "976us", "1.952ms",
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	/* It seems that soc_pcm_hw_params() in soc-core.c does not allow
	   us to have the option of fixed output rate. The rate in *params
	   is the one you passed in while you open the alsa device to play
	   a pcm song.
	   We are hard coding the output rate to 48k.
	*/
...
#else
	rate = 0;
	pr_debug("codec rate 48k.Original rate is %d\n",
		params_rate(params));
#endif
...
/*
* We are assigning channels_max = 6 for headset and speaker codec
* dais. This is because soc_pcm_open() in soc-core.c calculates
* the runtime->hw.channels_max using min() of codec and cpu (platform)
* dai, we cannot expose the 6 channels capability of our driver to ALSA.
* We will do downmix 6->2 channels in our plarform driver, and the actual
* max physical channels in here is 2.
*/
...
static void sn95031_jack_wq(struct work_struct *work)
{
	unsigned int mask = SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_HEADSET;
	struct sn95031_priv *sn95031_ctx =
	...
	if (jack_wq->intr_id & SN95031_JACK_INSERTED) {
		...
		* voltage level */
		if (sn95031_lp_flag) {
			...
			/* FIXME: Mic bias voltage different
				compared to phone */
			if (0 == voltage) {
