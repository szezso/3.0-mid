...
/* Per 3GPP TS 27.010, 5.7.2, maximum frame size is up to 32768 */
...
static void gsm_cleanup_mux(struct gsm_mux *gsm)
{
	...
	for (;;) {
		...
		prepare_to_wait(&gsm->event, &wait, TASK_UNINTERRUPTIBLE);

		for (i = 0; i < NUM_DLCI; i++)
			if (gsm->dlci[i] && gsm->dlci[i]->state != DLCI_CLOSED)
				count++;
...
static void gsmld_detach_gsm(struct tty_struct *tty, struct gsm_mux *gsm)
{
	...
	/*
	 * We need to drop ldisc_mutex here so that we can drop
	 * BTM in gsm_cleanup_mux w/o causing lock ordering issues;
	 * for most cases we should be protected by TTY_DLCI_CHANGING
	 * which is set for us in tty_set_ldisc().
	 */
