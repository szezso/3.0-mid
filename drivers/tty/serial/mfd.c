				...
				const struct pci_device_id *ent)
{
	...
	if (index == 3) {
		...
		do {
			...
			if (index != 3) {
				ret = request_irq(pdev->irq, port_irq, 0,
							uport->name, uport);
				if (ret) {
					dev_err(&pdev->dev, "can not get IRQ\n");
					goto err_disable;
...
#ifdef CONFIG_PM
void mfld_hsu_enable_wakeup(int index, struct device *dev,
				irq_handler_t wakeup);
...
static int hsu_runtime_suspend(struct device *dev)
{
	...
	memcpy(up->reg_shadow + 1, up->port.membase + 1,
			HSU_PORT_REG_LENGTH - 1);
	if (up->running)
