...
/* 100% = GAIN * 1.16(bias gause)*/
#define SELFTEST_XY_GAIN (SELFTEST_GAIN * 116 / 100)
/* BIAS 20% of GAIN*/
#define SELFTEST_XY_BIAS (SELFTEST_XY_GAIN * 20 / 100)

/* XY_MIN = GAIN * 1.16(bias gause) -  BIAS) */
#define SELFTEST_XY_MIN (SELFTEST_XY_GAIN - SELFTEST_XY_BIAS)
/* XY_MAX = GAIN * 1.16(bias gause) + BIAS) */
#define SELFTEST_XY_MAX (SELFTEST_XY_GAIN + SELFTEST_XY_BIAS)
#define SELFTEST_XY_VALID(xy) (xy > SELFTEST_XY_MIN && xy < SELFTEST_XY_MAX)

/* 100% = GAIN * 1.08(bias gause)*/
#define SELFTEST_Z_GAIN (SELFTEST_GAIN * 108 / 100)
/* BIAS 20% of GAIN*/
#define SELFTEST_Z_BIAS (SELFTEST_Z_GAIN * 20 / 100)

/* Z_MIN = GAIN * 1.08(bias gause) - BIAS */
#define SELFTEST_Z_MIN (SELFTEST_Z_GAIN - SELFTEST_Z_BIAS)
/* Z_MAX = GAIN * 1.08(bias gause) + BIAS */
#define SELFTEST_Z_MAX (SELFTEST_Z_GAIN + SELFTEST_Z_BIAS)
#define SELFTEST_Z_VALID(z) (z > SELFTEST_Z_MIN && z < SELFTEST_Z_MAX)

#define SELFTEST_X_ADJUST(vx) (magval_st[0] ? \
	(vx * SELFTEST_XY_GAIN / magval_st[0]) : vx)
#define SELFTEST_Y_ADJUST(vy) (magval_st[1] ? \
	(vy * SELFTEST_XY_GAIN / magval_st[1]) : vy)
#define SELFTEST_Z_ADJUST(vz) (magval_st[2] ? \
	(vz * SELFTEST_Z_GAIN / magval_st[2]) : vz)
...
#define HMC5883_POLL_DUR_MIN (1000 / HMC5883_MAX_RATE + 5)
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (pollms < HMC5883_POLL_DUR_MIN)
		pollms = HMC5883_POLL_DUR_MIN;
