...
/* adc to lux table
 * A translation table to covert raw adc value
 * to lux value: lux=pow(10, 0.08*adc). 0.08 is
 * decided according to adc values of 1000 LUX
 * and 100 LUX in hardware spec.
 */

#define DLS_ADC_MIN 0
#define DLS_ADC_MAX 63
#define DLS_ADC_CNT (DLS_ADC_MAX - DLS_ADC_MIN + 1)

static unsigned int adc_to_lux[DLS_ADC_CNT] = {
/* 0-9 */
1,	1,	1,	1,	2,	2,	3,	3,	4,	5,
/* 10-19 */
6,	7,	9,	10,	13,	15,	19,	22,	27,	33,
/* 20-29 */
39,	47,	57,	69,	83,	100,	120,	144,	173,	208,
/* 30-39 */
251,	301,	363,	436,	524,	630,	758,	912,	1096,	1318,
/* 40-49 */
1584,	1905,	2290,	2754,	3311,	3981,	4786,	5754,	6918,	8317,
/* 50-59 */
10000,	12022,	14454,	17378,	20892,	25118,	30199,	36307,	43651,	52480,
/* 60-63 */
63095,	75857,	91201,	109647
};
...
#define ALPS_WINDOW_LOSS_89     0xC
...
static int ambient_get_lum(int adc)
{
	if (adc < DLS_ADC_MIN)
		adc = DLS_ADC_MIN;
	else if (adc > DLS_ADC_MAX)
		adc = DLS_ADC_MAX;

	return adc_to_lux[adc - DLS_ADC_MIN];
}
...
static int ltr502als_initchip(struct alsps_device *alsps)
{
	...
	ret = alsps_read(alsps, DPSCTROL, &val);
	if (ret < 0)
		return ret;

	alsps_write(alsps, DPSCTROL, (val & PRX_COUNT_MASK) | 0x12);
	ret = alsps_write(alsps, CONFIGREG, POWER_UP | ACTIVE_DLPS);
	if (ret < 0)
		return ret;

	/* set low lux threshhod for 89% window loss */
	ret = alsps_read(alsps, DLSCTROL, &val);
	if (ret < 0)
		return ret;

	val &= ~LOW_LUX_MASK;
	val |= LOW_LUX_THRESH;
	ret = alsps_write(alsps, DLSCTROL, val);
	if (ret < 0)
		return ret;

	/* compensate 89% window loss */
	ret = alsps_write(alsps, DLSWINDOW,
			ALPS_WINDOW_LOSS_89 & ALPS_WINDOW_LOSS);
...
static void ltr502_mode(struct alsps_client *client, int mode)
{
	...
	case ALS_MODE_DISABLE:
		...
		if (--alsps_dev->ambient_count <= 0) {
			...
			/*
			 * Reset ALS value when disabling, otherwise the old
			 * value could be read out to user space until the new
			 * data comes after we re-enable ALS again.
			 */
			alsps_dev->als_state.now = 0;
...
static const struct file_operations ambient_fops = {
	.owner = THIS_MODULE,
	.open = ambient_open,
	.read = ambient_read,
	.poll = ambient_poll,
