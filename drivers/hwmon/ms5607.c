 ...
 * send reset sequence
 */
static void cmd_reset(struct i2c_client *client)
{
	u8 buf[7];
 ...
 */
static unsigned int cmd_prom(struct i2c_client *client, char coef_num)
{
	int err;
	u8 buf[7];
 ...
 */
static unsigned long cmd_adc(struct ms5607_data *ms5607,	char cmd)
{
	int err;
	u8 buf[7];
	unsigned long temp;
