...
static irqreturn_t otg_irq(int irq, void *_dev)
{
	...
	if (pnw->dev->power.is_suspended) {
		/*
		 * WA: since we use IRQF_NO_SUSPEND, it is possible
		 * scu fw may forward msi prior to otg resume function
		 * is called, in this case, we can ignore it and give scu
		 * EOI to allow things to continue.
		 * ideally, scu fw should differenciate wake-up events
		 * and normal interrupts.
		 */
		dev_dbg(pnw->dev, "Interrupt ignored in sys suspended\n");
		return IRQ_HANDLED;
	}
...
static void penwell_otg_stop_ulpi_poll(void)
{
	...
	/* we have to use this version because this function can potentially
				...
				unsigned long action, void *data)
{
	...
	case MID_OTG_NOTIFY_CRESET:
		...
		/* FIXME: This is not needed with Hub/Host chargers */
...
static void penwell_otg_work(struct work_struct *work)
{
	...
	case OTG_STATE_B_IDLE:
		if (hsm->id == ID_A || hsm->id == ID_ACA_A) {
			...
			if (charger_type == CHRG_DCP) {
				...
				/* WA: for MSIC issue need disable phy intr */
	...
	case OTG_STATE_B_PERIPHERAL:
		...
		if (hsm->id == ID_A) {
			...
			/* WA: reenable it for unplug event */
	...
	case OTG_STATE_A_IDLE:
		if (hsm->id == ID_B || hsm->id == ID_ACA_B) {
			...
			/* Wait for the host driver before going any further */
			if (!iotg->start_host)
				break;
...
static DEVICE_ATTR(a_clr_err, S_IWUSR|S_IWGRP, NULL, set_a_clr_err);
		...
		const struct pci_device_id *id)
{
	...
	wake_lock_init(&pnw->iotg.wake_lock, WAKE_LOCK_SUSPEND,
		pci_name(pdev));
	...
	if (request_irq(pdev->irq, otg_irq, IRQF_SHARED | IRQF_NO_SUSPEND,
				driver_name, pnw) != 0) {
		dev_dbg(pnw->dev,
			"request interrupt %d failed\n", pdev->irq);
		retval = -EBUSY;
...
static void penwell_otg_remove(struct pci_dev *pdev)
{
	...
	wake_lock_destroy(&pnw->iotg.wake_lock);
	kfree(pnw);
	pnw = NULL;
}

static void penwell_otg_shutdown(struct pci_dev *pdev)
{
	struct penwell_otg *pnw = the_transceiver;
