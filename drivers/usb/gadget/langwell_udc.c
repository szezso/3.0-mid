...
static void run_stop(struct langwell_udc *dev, int run)
{
	u32			usbcmd;
		...
		gfp_t gfp_flags)
{
	...
	if (!ep->desc) {
		...
		dev_err(&dev->pdev->dev,
			"ep disabled while enqueuing a request\n");
		return -EINVAL;
	}
...
static void langwell_ep_fifo_flush(struct usb_ep *_ep)
{
	...
	/* 3ms delay to guarantee the PHY really exits low power mode. */
	mdelay(3);
...
static int langwell_wakeup(struct usb_gadget *_gadget)
{
	...
	langwell_phy_low_power(dev, 0);
	/* Wait 3 milliseconds to avoid hitting fabric error */
	mdelay(3);
...
static int langwell_set_selfpowered(struct usb_gadget *_gadget, int value)
{
	struct langwell_udc *dev =
		container_of(_gadget, struct langwell_udc, gadget);
...
static const struct usb_gadget_ops langwell_ops = {
	...
	/* set the device selfpowered feature */
...
static int intel_mid_start_peripheral(struct intel_mid_otg_xceiv *iotg)
{
	...
	if (dev->stopped) {
		...
		if (dev->driver) {
			...
			if (dev->softconnected)
				langwell_udc_start(dev);
			else {
				u32 usbintr;
				/*
				 * FIXME: enable interrupts but don't pullup D+
				 * This is to avoid connection if the gadget has
				 * called otg_gadget_disconnect()
				 */
				usbintr = INTR_ULPIE | INTR_SLE | INTR_URE
					| INTR_AAE | INTR_SEE | INTR_FRE
					| INTR_PCE | INTR_UEE | INTR_UE;
				writel(usbintr, &dev->op_regs->usbintr);
				dev->stopped = 0;
			}
#else
		...
		*peripheral_driver)
{
	...
	langwell_udc_notify_otg(MID_OTG_NOTIFY_CLIENTREMOVE);
