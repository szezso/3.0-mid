				       ...
				       struct sep_queue_info **my_queue_elem)
{
	...
	if (sep->in_use) {
		sep->in_use = 0;
		wake_unlock(&sep->sep_wake_lock_suspend);
...
int sep_send_command_handler(struct sep_device *sep)
{
	...
	wake_lock(&sep->sep_wake_lock_suspend);
	...
	const struct pci_device_id *ent)
{
	...
	wake_lock_init(&sep->sep_wake_lock_suspend, WAKE_LOCK_SUSPEND,
		"sep_suspend_wake_lock");
...
end_function_free_sep_dev:
	wake_lock_destroy(&sep_dev->sep_wake_lock_suspend);
