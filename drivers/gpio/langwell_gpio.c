...
void lnw_gpio_set_alt(int gpio, int alt)
{
	...
	if (gpio < lnw->chip.base || gpio >= lnw->chip.base + lnw->chip.ngpio) {
		dev_err(lnw->chip.dev, "langwell_gpio: wrong pin %d to config"
						" alt\n", gpio);
		return;
	}
	if (lnw->irq_base + gpio - lnw->chip.base != gpio_to_irq(gpio)) {
		dev_err(lnw->chip.dev, "langwell_gpio: wrong chip data for"
					" pin %d\n", gpio);
		return;
	}
	gpio -= lnw->chip.base;
...
static int lnw_irq_wake(struct irq_data *d, unsigned int on)
{
	/* stub to support driver irq_wake usage, no actual support yet */
	return 0;
}
