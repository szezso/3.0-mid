...
static int ltr301_tune_threshold(struct ltr301_chip *chip)
{
	...
	avg_thresh = (chip->lo_thresh + chip->hi_thresh) / 2;

	if (chip->lux > avg_thresh)
		thresh_reg = ALS_THRES_UP_0;
	else
		thresh_reg = ALS_THRES_LOW_0;

	return ltr301_write_word(chip, thresh_reg, chip->ch0);
}
...
static irqreturn_t ltr301_interrupt(int irq, void *data)
{
	...
	if (status & ALS_ST_INT && status & ALS_ST_DATA) {
		...
		if (ltr301_check_threshold(chip)) {
...
static int ltr301_input_open(struct input_dev *dev)
{
	struct ltr301_chip *chip = input_get_drvdata(dev);

	mutex_lock(&chip->lock);
	i2c_smbus_write_byte_data(chip->client, ALS_CONTR, ALS_CT_MODE_ACTIVE);
	msleep(ALS_WAKEUP_TIME);
	chip->in_use = true;
	mutex_unlock(&chip->lock);
...
static void ltr301_input_close(struct input_dev *dev)
{
	struct ltr301_chip *chip = input_get_drvdata(dev);
				  ...
				  const struct i2c_device_id *id)
{
	...
	input_dev->open = ltr301_input_open;
	input_dev->close = ltr301_input_close;
